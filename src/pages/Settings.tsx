...
const Settings: React.FC = () => {
  ...
  const [selectedProvider, setSelectedProvider] = useState(llmConfig.provider);
  ...
  const isZeroTokenProvider = selectedProvider === 'zero-token';

  const handleProviderChange = (value: string) => {
    setSelectedProvider(value);
    setLlmConfig({ provider: value });
  };
              ...
              <Card>
                <Form form={form} layout="vertical" initialValues={llmConfig}>
                  <Form.Item name="provider" label="API 提供商" rules={[{ required: true }]}>
                    <Select placeholder="选择 API 提供商" onChange={handleProviderChange}>
